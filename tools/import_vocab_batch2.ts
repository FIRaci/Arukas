...
const OUTPUT_PATH = resolve(ROOT, 'src/data/vocabImportBatch2.ts');
...
function buildBatch(): { wordsByLevel: Record<Level, string[]>; entriesByLevel: Record<Level, VocabEntry[]> } {
  ...
  for (const raw of rawItems) {
    ...
    const id = `vimp2-${level.toLowerCase()}-${String(counters[level]).padStart(4, '0')}`;
...
function writeBatchFile(wordsByLevel: Record<Level, string[]>, entriesByLevel: Record<Level, VocabEntry[]>): void {
  ...
  file.push('// ============================================================================');
  file.push('// VOCAB IMPORT BATCH 2 — Generated from tools/vocab_new.txt');
  file.push('// Scope: remaining N5-N1 entries not yet present in current dataset');
  file.push('// ============================================================================');
  file.push("import type { VocabEntry } from './vocabTypes';");
  file.push('');

  for (const level of LEVELS) {
    file.push(printWordArray(`VOCAB_IMPORT_BATCH2_${level}_WORDS`, wordsByLevel[level]));
    file.push('');
  }

  for (const level of LEVELS) {
    file.push(printEntryArray(`VOCAB_IMPORT_BATCH2_${level}`, entriesByLevel[level]));
    file.push('');
  }
