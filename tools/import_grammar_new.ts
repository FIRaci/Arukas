...
const OUTPUT_PATH = resolve(ROOT, 'src/data/grammarImportBatch1.ts');
const LEVELS = ['N2', 'N1'] as const;

type ImportLevel = (typeof LEVELS)[number];
...
function parseJsonl(path: string): RawGrammar[] {
  ...
  for (const lineRaw of lines) {
    ...
    try {
      const parsed = JSON.parse(line) as RawGrammar;
      out.push(parsed);
    } catch {
...
function makeNote(usage?: string, construction?: string): string | undefined {
  const pieces: string[] = [];
  if (usage && usage.trim()) {
    pieces.push(`Usage: ${usage.trim()}`);
  }
  if (construction && construction.trim()) {
    pieces.push(`Construction: ${construction.trim()}`);
  }
  if (pieces.length === 0) {
    return undefined;
  }
...
function buildImportBatch(): { batch: Record<ImportLevel, GrammarEntry[]>; skippedExisting: number; skippedInvalid: number } {
  ...
  const batch: Record<ImportLevel, GrammarEntry[]> = { N2: [], N1: [] };
  const counters: Record<ImportLevel, number> = { N2: 1, N1: 1 };

  let skippedExisting = 0;
  let skippedInvalid = 0;

  for (const row of raw) {
    const level = (row.jlpt ?? '').trim().toUpperCase() as ImportLevel;
    ...
    const id = `gimp-${level.toLowerCase()}-${String(counters[level]).padStart(3, '0')}`;
    counters[level] += 1;
    ...
    if (examples.length > 0) {
      entry.examples = examples.slice(0, 4);
    }
  ...
  return { batch, skippedExisting, skippedInvalid };
}

function writeBatchFile(batch: Record<ImportLevel, GrammarEntry[]>): void {
  const file: string[] = [];

  file.push('// ============================================================================');
  file.push('// GRAMMAR IMPORT BATCH 1 — Generated from tools/grammar_new.txt');
  file.push('// Scope: N2 + N1 only, deduped by normalized pattern against existing dataset');
  file.push('// ============================================================================');
  file.push("import type { GrammarEntry } from './grammarTypes';");
  file.push('');
  file.push('export const GRAMMAR_IMPORT_BATCH1_N2: GrammarEntry[] = [');
  for (const entry of batch.N2) {
    file.push(entryToTs(entry));
  }
  file.push('];');
  file.push('');
  file.push('export const GRAMMAR_IMPORT_BATCH1_N1: GrammarEntry[] = [');
...
function main(): void {
  const { batch, skippedExisting, skippedInvalid } = buildImportBatch();
  writeBatchFile(batch);

  console.log('Generated:', OUTPUT_PATH);
  console.log(`N2 imported: ${batch.N2.length}`);
  console.log(`N1 imported: ${batch.N1.length}`);
  console.log(`Skipped existing/duplicates: ${skippedExisting}`);
